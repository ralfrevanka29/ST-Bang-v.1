...
#define UNIT_DELAY      23
#define SCROLL_DELAY    (3 * UNIT_DELAY)
#define MIDLINE_DELAY   (3 * UNIT_DELAY)
#define SCANNER_DELAY   (2 * UNIT_DELAY)
#define SPECTRUM_DELAY  (3 * UNIT_DELAY)
#define HEARTBEAT_DELAY (2 * UNIT_DELAY)
#define HEARTS_DELAY    (3 * UNIT_DELAY)
#define EYES_DELAY      (3 * UNIT_DELAY)
#define WIPER_DELAY     (3 * UNIT_DELAY)
#define INVADER_DELAY   (2 * UNIT_DELAY)
#define PACMAN_DELAY    (2 * UNIT_DELAY)
#define SINE_DELAY      (3 * UNIT_DELAY)

#define CHAR_SPACING     1  // pixels between characters
#define BUF_SIZE        75  // character buffer size

// ========== General Variables ===========
const char* ssid = "ST-BANG";
...
void setupWiFi() {
  ...
  server.on("/", HTTP_GET, [](void) {
    ...
    html += "<input type=\"text\" name=\"msg\" placeholder=\"Enter your message\">";
    html += "<h2>MODE</h2>";
    html += "<input type=\"radio\"name=\"radio\" value=\"runningtext\">Running Text";
    html += "<input type=\"radio\"name=\"radio\" value=\"animasi\">Animasi";
  ...
  server.on("/message", HTTP_POST, []() {
    if (server.hasArg("radio")) {
      String radio = server.arg("radio");
      if(radio == "animasi"){
         mode = radio.c_str(); // Update the message in the msgT
         text ="";
        ...
        String radio = server.arg("radio");
        text = msg.c_str(); // Update the message in the msgTab
        mode = radio.c_str(); // Update the message in the msgTab
      }
...
bool scrollText(bool bInit, const char *pmsg)
{
  ...
  static unsigned long prevTimeAnim = 0;  // Add missing declaration
  uint8_t         colData;
...
// the mode switch is pressed.
{
  ...
  if (changeState)
  {
    ...
    {
      mesg++;
      if (mesg >= sizeof(text)/sizeof(text[0]))
      {
        mesg = 0;
    	bInMessages = false;
        state++;
  ...
  switch(state)
  {
    ...
    case  0: bInMessages = true; bRestart = scrollText(bRestart, text.c_str()); break;

    default: state = 0;
  }
}
void runMatrixAnimation1(void)
// Schedule the animations, switching to the next one when the
// the mode switch is pressed.
{
  static  uint8_t state = 0;
  ...
  switch(state)
  {
    ...
    case  5: bRestart = graphicHeartbeat(bRestart);      break;
    case  6: bRestart = graphicHearts(bRestart);         break;
    case  7: bRestart = graphicEyes(bRestart);           break;
